...
public class UpdateUserHandlerTests
{
    ...
    {
        // Arrange
        var command = new UpdateUserCommand { Id = 1, Name = "New Name", Email = "new@example.com" };
    ...
    [Fact]
    public async Task Handle_WithPassword_ShouldResetPassword()
    {
        // Arrange
        var command = new UpdateUserCommand { Id = 1, Password = "NewPassword123" };
        var user = new DomainUser { Id = 1, Name = "Name" };
        ...
        _userManagerMock.Setup(m => m.ResetPasswordAsync(user, "token", command.Password))
            .ReturnsAsync(IdentityResult.Success);
        ...
        _userManagerMock.Verify(m => m.ResetPasswordAsync(user, "token", command.Password), Times.Once);
    }

    [Theory]
    [InlineData("", "email@test.com", "password123", "User name cannot be empty.")]
    [InlineData("Name", "", "password123", "User email cannot be empty.")]
    [InlineData("Name", "email@test.com", "short", "Password must be at least 8 characters long.")]
    public async Task Handle_InvalidInput_ShouldThrowBusinessException(string name, string email, string password, string expectedMessage)
    {
        // Arrange
        var command = new UpdateUserCommand { Id = 1, Name = name, Email = email, Password = password };
    ...
    {
        // Arrange
        var command = new UpdateUserCommand { Id = 1, Name = "Name" };
        var user = new DomainUser { Id = 1, Name = "Name" };
        _userManagerMock.Setup(m => m.FindByIdAsync("1"))
            .ReturnsAsync(user);
        _userManagerMock.Setup(m => m.UpdateAsync(user))
